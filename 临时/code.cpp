...
struct LB{
    ll a[M],_a[M];
    void Clear() { mem(a,0),mem(_a,0); }
    void Copy(LB &A) { rep(i,0,M) a[i]=A.a[i]; }
    bool I(ll x,ll &y) {
        y=x;
        for(int i=M-1; ~i && x; --i) if (x>>i&1)
            if (a[i]) x^=a[i],y^=_a[i]; else { a[i]=x,_a[i]=y; return 1; }
        return 0;
    }
    bool Q(ll x) {
        for(int i=M-1; ~i && x; --i) if (x>>i&1) if (a[i]) x^=a[i];
        return !x;
    }
    friend void Merge(LB &A,LB &B,LB &C) {
        LB AA; ll y,z; AA.Copy(A),C.Clear();
        rep(i,0,M) if (B.a[i]) if (!AA.I(B.a[i],y)) C.a[i]=y;
    }
...
void build(int t,int l,int r) {
    if (l==r) {
        for (auto x: V[l]) B[t].I(x,y); mem(B[t]._a,0);
//        B[t].print();
        return;
    }
    int mid=l+r>>1;
...
bool query(int t,int l,int r,int L,int R,ll x) {
    ...
    if (L<=mid) return ret&=query(t<<1,l,mid,L,R,x);
    if (R>mid) return ret&=query(t<<1|1,mid+1,r,L,R,x);
    return ret;
}
